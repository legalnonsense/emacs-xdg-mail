...
For various reasons, I could not get the .desktop files that come with emacs to work with mailto: links with my mu4e and org-msg set up. This fixes that. I have provided:
...
8. This should make mailto: links work system wide (file manager, libreoffice (File -> Send -> Email document), firefox, etc.), but if you are using KDE, XFCE, Gnome, etc., then there may be an additional way to set the default mail application in the DE’s settings manager. If so, simply set it to =emacs-xdg-email=. 
